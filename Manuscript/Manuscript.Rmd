...
### make long ###
data.long <- gather(data, key = "Condition", value = "ReactionTimeZ",
                    lo.neu.sur_n_RTz, hi.neu.sur_n_RTz,
                    lo.emo.sur_n_RTz, hi.emo.sur_n_RTz,
                    lo.neu.sur_p_RTz, hi.neu.sur_p_RTz,
                    lo.emo.sur_p_RTz, hi.emo.sur_p_RTz)
data.long$Condition <- as.factor(data.long$Condition)

data.long.summary <- summarySE(data.long, measurevar="ReactionTimeZ", groupvars="Condition", na.rm = TRUE)

ggplot(data.long.summary, aes(Condition, ReactionTimeZ, fill = as.factor(Condition))) +
  geom_bar(stat = "summary", fun.y = "mean", na.rm = TRUE) +
  geom_errorbar(aes(ymin=ReactionTimeZ-se, ymax=ReactionTimeZ+se), width=.1)
